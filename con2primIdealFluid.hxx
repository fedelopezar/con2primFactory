...
namespace AsterX
{
    ...
    public:
        ...
        CCTK_HOST CCTK_DEVICE inline void get_ConservedVarsFromPrimVector(CCTK_REAL (&prim)[NPRIMS]);
    ...
    inline void idealFluid::get_ConservedVarsFromPrimVector(CCTK_REAL (&prim)[NPRIMS])
    {
        // FIX THIS!!!
        ...
        bsq_Seed = (Bsq + (alp * bt) * (alp * bt)) / (w_lorentz * w_lorentz);

        ConservedVars[D] = sqrt_gdet * prim[RHO] * w_lorentz;

        ConservedVars[S1_COV] = sqrt_gdet * (w_lorentz * w_lorentz * (prim[RHO] + prim[RHO] * prim[EPS] * GammaIdealFluid + bsq_Seed) * v1_Valencia_cov - alp * bt * bx_cov);

        ConservedVars[S2_COV] = sqrt_gdet * (w_lorentz * w_lorentz * (prim[RHO] + prim[RHO] * prim[EPS] * GammaIdealFluid + bsq_Seed) * v2_Valencia_cov - alp * bt * by_cov);

        ConservedVars[S3_COV] = sqrt_gdet * (w_lorentz * w_lorentz * (prim[RHO] + prim[RHO] * prim[EPS] * GammaIdealFluid + bsq_Seed) * v3_Valencia_cov - alp * bt * bz_cov);

        ConservedVars[TAU] = sqrt_gdet * (w_lorentz * w_lorentz * (prim[RHO] + prim[RHO] * prim[EPS] * GammaIdealFluid + bsq_Seed) - (prim[EPS] * prim[RHO] * (GammaIdealFluid - 1.0) + bsq_Seed / 2.0) - alp * alp * bt * bt - ConservedVars[D]);
